...
app.secret_key="@lucky"#alphanumeric
...
def register():
    ...
    if request.method=="POST":
        ...
        if code==ccode:
            ...
            cursor.execute('insert into students values(%s,%s,%s,%s)',(rollno,name,group,password))
...
def filehome():
    if session.get('user'):
        ...
        if request.method=='POST':
            ...
            cursor=mysql.connection.cursor()
            cursor.execute('insert into files(rollno,filename,fileupload)values(%s,%s,%s)',[session.get('user'),filename,fileupload])
            mysql.connection.commit()
            #We add these lines to make visible
            cursor.execute('select * from files where rollno=%s',[session.get('user')])
            filesdata=cursor.fetchall()
            cursor.close()
        return render_template('fileuploadtable.html',filesdata=filesdata)
    else:
        return redirect(url_for('login'))
...
def readfile(fid):
     if session.get('user'):
         cursor=mysql.connection.cursor()
         cursor.execute('select filename,fileupload from files where fid=%s',[fid])
         filesdata=cursor.fetchone()
         cvt_bytes=BytesIO(filesdata[1])
         return send_file(cvt_bytes,download_name=filesdata[0])
     else:
         return redirect(url_for('login'))
